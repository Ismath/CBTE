...
from .models import t_user, t_products, t_product_provider, t_consumptions, t_announcements


class T_userSerializer(serializers.ModelSerializer):
    class Meta:
        model = t_user
        fields = ('user_id','username','secret','email','full_name','nic','address','telephone','type','verified','creation_date','modification_date')
...
class T_consumptionSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ('id', 'rating', 'status', 'enabled', 'start_time', 'end_time', 'creation_date', 'modification_date', 't_product_provider_id', 'user_id')
